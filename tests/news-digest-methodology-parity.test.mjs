...
function extractYamlSchemaBlock(yamlText, schemaName) {
  ...
  // biome-ignore lint/complexity/useIndexOf: the index itself is used below to slice the block — .includes() cannot replace it
  const start = lines.findIndex((line) => line === `        ${schemaName}:`);
  assert.ok(start !== -1, `failed to locate YAML schema ${schemaName}`);
