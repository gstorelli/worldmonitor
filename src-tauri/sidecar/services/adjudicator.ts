...
export interface AdjudicatorResponse {
  customs_relevance_score: number;
  reasoning: string;
}

const sleep = (ms: number) => new Promise(r => setTimeout(r, ms));

async function fetchFromOpenRouter(model: string, userPrompt: string): Promise<any> {
 ...
 */
export async function adjudicateEvent(eventId: string, eventTitle: string, eventDesc: string): Promise<AdjudicatorResponse | null> {
