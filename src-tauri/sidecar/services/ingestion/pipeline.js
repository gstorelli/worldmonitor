...
export async function runIngestionPipeline() {
    ...
    for (const event of events) {
        ...
        try {
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?)
        `).run(event.id, event.title, event.description + (reasoning !== 'Rule-based scoring' ? `\n[AI Reason: ${reasoning}]` : ''), event.sourceUrl, finalScore / 100, // Normalized to 0-1 for UI
                confidence, embeddingId);
                insertedCount++;
            }
        }
        catch (dbError) {
